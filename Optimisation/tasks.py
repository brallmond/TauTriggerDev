...
#class SaveOptimisationResultsVBFSingleTau(SaveOptimisationResults):
class SaveOptimisationResultsVBFSingleTau(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    # I added these 4 lines from the other function
    def output(self):
        p = self.branch_data
        path = os.path.join(self.output_path, f'results_{"_".join([str(elem) for elem in p])}.json')
        return law.LocalFileTarget(path)
    ...
    def run(self):
        ...
        HLT_name = "HLT_VBF_DiPFJet45_Mjj650_MediumDeepTauPFTauHPS45_L2NN_eta2p1"
        L1A_physics = float(self.config['RUNINFO']['L1A_physics'])
        ...
        # FileNameList_eff = f"/eos/user/j/jleonhol/www/PNetAtHLT/data/EfficiencyDen/{HLT_name}/ZprimeToTauTau_M-4000.root" 
        FileNameList_eff = [
            # not clear why hard-coded?
            f"/afs/cern.ch/work/b/ballmond/PNetAtHLT/data/EfficiencyDen/HLT_VBF_DiPFJet45_Mjj650_MediumDeepTauPFTauHPS45_L2NN_eta2p1/VBFHToTauTau_M125.root",
            #f"/afs/cern.ch/work/b/ballmond/PNetAtHLT/data/EfficiencyDen/HLT_VBF_DiPFJet45_Mjj650_MediumDeepTauPFTauHPS45_L2NN_eta2p1/GluGluHToTauTau_M-125.root",
            #f"/afs/cern.ch/work/b/ballmond/PNetAtHLT/data/EfficiencyDen/HLT_VBF_DiPFJet45_Mjj650_MediumDeepTauPFTauHPS45_L2NN_eta2p1/GluGlutoHHto2B2Tau_kl-1p00_kt-1p00_c2-0p00.root",
            #f"/eos/user/j/jleonhol/www/PNetAtHLT/2024_v1/data/EfficiencyDen/{HLT_name}/VBFHToTauTau_M125.root"
        ...
        if not "deeptau" in params:
            N_den, N_num = dataset_rate.get_Nnum_Nden_VBFSingleTauPNet(params)
            rate = (N_num/N_den)*L1A_physics

            N_den, N_num = dataset_eff.ComputeEffAlgo_VBFSingleTauPNet(params)
            eff = (N_num/N_den)
        else:
            N_den, N_num = dataset_rate.get_Nnum_Nden_HLTVBF_DiPFJet45_Mjj650_MediumDeepTauPFTauHPS45_L2NN_eta2p1()
            rate = (N_num/N_den)*L1A_physics

            N_den, N_num = dataset_eff.ComputeEffAlgo_HLTVBF_DiPFJet45_Mjj650_MediumDeepTauPFTauHPS45_L2NN_eta2p1()
            eff = (N_num/N_den)
...
class SaveOptimisationResultsDiTauSingleTau(SaveOptimisationResultsDiTau):
    ...
    def run(self):
        ...
        params = self.branch_map[self.branch]
        if not "deeptau" in params:
            N_den, N_num = dataset_rate.get_Nnum_Nden_DoubleORSinglePNet(self.par_double, params)
            rate = (N_num/N_den)*L1A_physics

            N_den, N_num = dataset_eff.ComputeEffAlgo_DoubleORSinglePNet(self.par_double, params, self.den_double)
            eff = (N_num/N_den)
        else:
